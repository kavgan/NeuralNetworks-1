...
public class Board {
	...
	boolean[][] breadCrumbs; //The player leaves breadcrumbs wherever they walk
	...
	public Board(int s) {
		...
		breadCrumbs = new boolean[size][size];
		for(int i = 0; i < size; i++) {
			for(int j = 0; j < size; j++) {
	...
	//returns the total amount of gold (at initialization)
	...
	//returns whether or not the player has visited their current spot
	public boolean getBreadCrumbs() {
		return breadCrumbs[pX][pY];
	}
	
	//returns the two dimensional array of bread crumbs in the sight range
	public boolean[][] getBreadCrumbs(int size) {
		//if whole board is visible
		if(this.size == size)
			return breadCrumbs;
		int sr = (size - 1)/2; //sight range
		//get the top and left coordinates of the visible range
		int top = pY - sr;
		if(top < 0)
			top += size;
		int left = pX - sr;
		if(left < 0)
			left += size;
		boolean[][] bc = new boolean[size][size];
		//add the breadcrumbs to the return array
		for(int i = 0; i < size; i++) {
			int y = top + i;
			if(y > size)
				y -= size;
